  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Edit Complaint</h1>
          <p className="mt-1 text-sm text-slate-400">Update the details of this complaint.</p>
        </div>
        <Link href={`/complaints/${params.id}`} className="text-sm font-medium text-slate-400 hover:text-white transition-colors">
          Cancel
        </Link>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <select
              ...
              disabled={!isAdmin}
              className="mt-1 block w-full rounded-lg border border-white/10 bg-[#0a0a0f] py-2.5 pl-3 pr-10 text-white shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm disabled:bg-white/5 disabled:text-slate-500 transition-colors"
            >
              {CATEGORIES.map((cat) => (
            ...
            {!isAdmin && <p className="text-xs text-slate-500 mt-2">Only admins can change the category.</p>}
          </div>
            ...
            <div className="flex items-center justify-center w-full">
              {preview ? (
                <div className="relative w-full h-48 rounded-lg overflow-hidden border border-white/10 bg-white/5">
                  ...
                  <img src={preview} alt="Preview" className="object-contain w-full h-full opacity-90" />
                  <button
                    type="button"
                ...
                <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-white/10 border-dashed rounded-lg cursor-pointer bg-white/5 hover:bg-white/10 transition-colors">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm text-slate-400"><span className="font-semibold text-white">Click to upload new photo</span> or drag and drop</p>
